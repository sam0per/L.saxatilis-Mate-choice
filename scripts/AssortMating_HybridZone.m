...
%Parameters.
%%%%%%%%%%%%%%%%%
%Choose the ID of the simulation results with random mating (ID is called
%partII here, and it is set to 1 for simplicity, but it can take values
%from 1 to 200).
partII=1;
...
numMate=100;
%Parameters of the assortative-mating function (Eq. (1) in the article).
%The parameters used in Eq. (1) are called here as follows: b_0 is called
%level, b_1 is called scale, c is called preference, d is called
%choosiness, and alpha is called asymmetry 
level=0.01;%the same value in all 4 models with assortment 
scale=0.4;%the same value in all four models of assortment
choosiness=0.85;%the same value in all four models with assortment
%In the AS model, the preference parameter is set to -0.17. In the SimMR
%model, this parameter is set to 0.3859. In the SimOR model, this parameter
%is set to 0.2671. In the SimM0 model, this parameter is set to 0. 
preference=0.3859;
%In the AS model, the asymmetry parameter is set to 2.33; in the remaining
%three models (SimMR, SimOR, and SimM0, that are symmetrical), asymmetry is
%set to 0.
asymmetry=0;

% totCond=10;

%partI is a dummy here; only specifies which initial condition (result from
%the random-mating model) is to be used. 
partI=part;

%load the corresponding initial condition
    ...
    count=0;    
    pop1M=reshape(pop1MALL(end,:,:,:),M,2*L,[]);
    pop1F=reshape(pop1FALL(end,:,:,:),M,2*L,[]);
    
    Phen1F=reshape(PhenF(end,:,:),M,[]);
    Phen1M=reshape(PhenM(end,:,:),M,[]);
    
%neutr_time: set to 0 here, because the burn-in period with random-mating
%is modelled separately and used here as an initial condition.
neutr_time=0;
...
for I=1:repeat
    ...
    %allow for finite number of mating attempts, with the probability of
    %success being given by pI.
    
    
matedMalesPhen1=zeros(1,M);
AllMalesPhen1=zeros(1,M);
    for i=1:M
        ...
        ratio=b-c;
        pI=(level+scale*(exp(-(ratio-preference).^2/(2*choosiness^2)))).*(1+erf(asymmetry*(ratio-preference)/(sqrt(2)*choosiness)));
        ...
        a=a+reshape(a2,1,[]);
        [~,b]=histc(a,boxesMate);
        b1=(0:(num_popF(1,i)-1))*(num_popM(1,i)+1);
        ...
        %compute fitnesses
        %since selection acts on phenotypes, that are now determined
...
%start with females
    ...
    %done with all parts in one lifecycle
...
filename=sprintf('AssortMatingPhase_ModelXXX_sTot%g_N%g_M%g_L%g_r%g_sigmaD%g_theta%g_numMate%g_sigmaE%g_factor%g_partI%g', sTot, N, M, L, r1, sigmaD, theta1, numMate, sigmaE, factor, partI);
%for "ModelXXX" choose the appropriate name according to the parameters set
%above.
filename=strrep(filename,'.','p');
save(filename,'level','scale','choosiness','preference','asymmetry','LDF','LDM','LDALL','LDFrel','LDMrel','LDALLrel','matedMalesPhen','AllMalesPhen','factor','pop1FALL','pop1MALL','transition','AlSize1','AlSize2','sTot','sigma','sel_time','sigmaMuSel','r1','r0','L','repeat','sigmaD','N','sigmaMu','mu','span','M','theta1','theta2', 'numMate','clineAlF','clineAlAll', 'clineAlM','PhenF','PhenM', 'sigmaE');            
