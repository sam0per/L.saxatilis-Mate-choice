...
if (is.null(opt$modelone) | is.null(opt$modeltwo)){
  print_help(opt_parser)
  stop("At least two arguments must be supplied (first and second model).\n", call.=FALSE)
}
...
comp1_2 <- compare(loo_1, loo_1)
print(paste0("estimated difference of expected leave-one-out prediction errors\nbetween ",
             basename(opt$modelone), " and ", basename(opt$modelone), " along with the standard error.\n",
