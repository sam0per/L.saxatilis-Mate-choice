...
transformed parameters {
  ...
  for (i in 1:N) {
    y_hat[i] = 0.01 + b_par * exp(-0.5 * ((ratio[i] - c_hyp[i]) / d_par)^2) * (1 + erf(alpha_par * (ratio[i] - c_hyp[i]) / (1.414214 * d_par)));
  }
}
...
generated quantities {
  vector[N] log_lik;
  for (n in 1:N) {
    log_lik[n] = bernoulli_logit_lpmf(y[n] | logit(0.01 + b_par * exp(-0.5 * ((ratio[n] - c_hyp[n]) / d_par)^2) * (1 + erf(alpha_par * (ratio[n] - c_hyp[n]) / (1.414214 * d_par)))));
  }
}
