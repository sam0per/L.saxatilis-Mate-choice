...
transformed parameters {
  ...
  for (i in 1:N) {
    y_hat[i] = 0.01 + b_par * exp(-0.5 * ((ratio[i] - c_par) / d_par)^2) * (1 + erf(alpha_hyp[i] * (ratio[i] - c_par) / (1.414214 * d_par)));
  }
}
...
generated quantities {
  vector[N] log_lik;
  for (n in 1:N) {
    log_lik[n] = bernoulli_logit_lpmf(y[n] | logit(0.01 + b_par * exp(-0.5 * ((ratio[n] - c_par) / d_par)^2) * (1 + erf(alpha_hyp[n] * (ratio[n] - c_par) / (1.414214 * d_par)))));
  }
}
