...
transformed parameters {
  ...
  for (i in 1:N) {
    y_hat[i] = 0.01 + b_par * exp(-0.5 * ((ratio[i] - c_par) / d_hyp[i])^2) * (1 + erf(alpha_par * (ratio[i] - c_par) / (1.414214 * d_hyp[i])));
  }
}
...
generated quantities {
  vector[N] log_lik;
  for (n in 1:N) {
    log_lik[n] = bernoulli_logit_lpmf(y[n] | logit(0.01 + b_par * exp(-0.5 * ((ratio[n] - c_par) / d_hyp[n])^2) * (1 + erf(alpha_par * (ratio[n] - c_par) / (1.414214 * d_hyp[n])))));
  }
}
